...
def main():
    ...
    for line in data:
        ...
        # non-overlapping)
        # any overlapping repeats are added to regions
        if chrom != cur_chrom or start > max_stop:
            ...
            max_index   = -1
            cov_frac    = 0.0
            # identify region with the max score
            for index,region in enumerate(regions):
                if region[5] >= max_score:
                    max_index = index
                    max_score = region[5]
                    # fraction of entire region covered by current repeat
                    cov_frac  = 1.0*(region[2]-region[1]+1)/region_size

            # region only 'passes' if one repeat takes up >85% of it
            # or if subregion period lengths are identical
            if max_index != -1:
